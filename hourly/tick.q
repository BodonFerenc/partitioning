...
ld:{if[not type key L::`$(":",(get `..src)),string x;.[L;();:;()]];i::j::-11!(-2;L);if[0<=type i;-2 (string L)," is a corrupt log. Truncate to length ",(string last i)," and restart";exit 1];hopen L};
tick:{init[];if[not min(`time`sym~2#key flip value@)each t;'`timesym];@[;`sym;`g#]each t;h::hour .z.P;if[l::count y;L::`$":",y,"/",x,10#".";l::ld h]};
...
if[system"t";
 ...
 upd:{[t;x]
 if[not -16=type first first x;if[h<hour a:.z.P;.z.ts[]];x:$[0>type first x;a,x;(enlist(count first x)#a),x]];
